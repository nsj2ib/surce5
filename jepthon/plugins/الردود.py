...
async def add_new_filter(new_handler):
    ...
    if msg and msg.media and not string:
        ...
        string = rep_msg.text
    success = "**𓆝︙ الـرد {} تـم اضـافتة بنـجـاح ✓**"
...
async def on_snip_list(event):
    ...
    OUT_STR = "**𓆝︙لا توجد ردود مضافة في هذه الدردشه  🔍**"
    filters = get_filters(event.chat_id)
    for filt in filters:
        if OUT_STR == "**𓆝︙لا توجد ردود مضافة في هذه الدردشة  🔍**":
            OUT_STR = "**𓆝︙الردود التي تم اضافتها في هذه الدردشة**\n"
        OUT_STR += "⌯︙{}\n".format(filt.keyword)
    await edit_or_reply(
        event,
        OUT_STR,
...
async def remove_a_filter(r_handler):
    ...
    if not remove_filter(r_handler.chat_id, filt):
        await r_handler.edit("**𓆝︙الـرد {} غير موجود **".format(filt))
    else:
        await r_handler.edit("**𓆝︙ الـرد {} تـم حـذفة بنـجـاح ✓**".format(filt))
...
async def on_all_snip_delete(event):
    ...
    if filters:
        ...
        await edit_or_reply(event, f"**𓆝︙ تم حذف الردود في الدردشة الحالية بنجاح ✓**")
    else:
        await edit_or_reply(event, f"𓆝︙لا توجد ردود في هذه المجموعة  ")
