...
async def _(event):  # sourcery no-metrics
    ...
    try:
        ...
        return await edit_delete(
            event, f"<b>𓆜︙ خـطـأ : </b><code>{str(e)}</code>", time=5, parse_mode="HTML"
        )
    ...
    totalstring = f"<code><b>𓆜︙ جمـيع الـملفات ☆ </b>       | {str(totalcount)}\
                  \n𓆜︙ حجـم الـملف الكـلي ☆    | {humanbytes(totalsize)}\
                  \n𓆜︙ حـجم الـملف ☆    | {avghubytes}\
                  \n</code>"
    runtimestring = f"<code>𓆜︙ وقـت الـتشغيل ☆            | {runtime}\
                    \n𓆜︙ وقـت الـتشغيل لـكل مـلف ☆    | {avgruntime}\
                    \n</code>"
    line = "<code>+--------------------+-----------+</code>\n"
    result = f"<b>𓆜︙ الـكـروب : {link}</b>\n\n"
    result += f"<code>𓆜︙ جـميـع الـرسائـل: {msg_count}</code>\n"
    result += "<b>𓆜︙ مـلخص الـملف : </b>\n"
...
async def _(event):  # sourcery no-metrics
    ...
    totalstring = f"<code> 𓆜︙ إجمالـي الملفـات ☆ :    | {str(totalcount)}\
                  \n 𓆜︙ الحجـم الإجمالـي للملـف ☆ :   | {humanbytes(totalsize)}\
                  \n 𓆜︙ حجم الملف  :  | {avghubytes}\
                  \n</code>"
    runtimestring = f"<code> 𓆜︙ وقـت التشغيـل ☆ :            | {runtime}\
                    \n وقـت التشغيـل لـكـل ملـف ☆ : | {avgruntime}\
                    \n</code>"
    line = "<code>+--------------------+-----------+</code>\n"
    result = f"𓆜︙ المجموعـة ☆ : {link}\nUser : {_format.htmlmentionuser(userdata.first_name,userdata.id)}\n\n"
    result += f"<code>𓆜︙ مجمـوع الرسائـل ☆ : {msg_count}</code>\n"
    result += "⌔︙ملخـص الملـف ☆ :\n"
    result += f"<code>{str(x)}</code>\n"
    result += f"{largest}"
    result += line + totalstring + line + runtimestring + line
