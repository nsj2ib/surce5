...
async def ytdl_down(event, opts, url):
    try:
        await event.edit("𓆜︙ - يتم جلب البيانات انتظر قليلا")
        ...
        await event.edit("𓆜︙ - عذرا هذا المحتوى قصير جدا لتنزيله ⚠️")
        return None
    except GeoRestrictedError:
        await event.edit(
            "𓆜︙ - الفيديو غير متاح من موقعك الجغرافي بسبب القيود الجغرافية التي يفرضها موقع الويب ❕"
        )
        return None
    except MaxDownloadsReached:
        await event.edit("𓆜︙ - تم الوصول إلى الحد الأقصى لعدد التنزيلات ❕")
        return None
    except PostProcessingError:
        await event.edit("𓆜︙ كان هناك خطأ أثناء المعالجة")
        return None
        ...
        await event.edit("𓆜︙ حدث خطأ أثناء استخراج المعلومات يرجى وضعها بشكل صحيح ⚠️")
        return None
    except Exception as e:
        await event.edit(f"𓆜︙ حدث خطا : \n__{str(e)}__")
        return None
    return ytdl_data
...
async def download_audio(event):
    ...
    if not url:
        return await edit_or_reply(event, "𓆜︙ - يجب وضع رابط لتحميله ❕")
    catevent = await edit_or_reply(event, "𓆜︙ يتم الاعداد انتظر")
    ...
    if ytdl_data is None:
        ...
        return
    await catevent.edit(
        f"𓆜︙ يتم لتحميل الأغنية:\
        \n𓆜︙ {ytdl_data['title']}\
        \nبواسطة 𓆜︙ {ytdl_data['uploader']}"
    )
...
async def download_video(event):
    ...
    if not url and rmsg:
        ...
        url = re.search("(?P<url>https?://[^\s]+)", myString).group("url")
    if not url:
        return await edit_or_reply(event, "𓆜︙ عـليك وضع رابـط اولا ليتـم تنـزيله")
    catevent = await edit_or_reply(event, "𓆜︙ يتم التحميل انتظر قليلا")
    reply_to_id = await reply_id(event)
...
async def yt_search(event):
    ...
    if not query:
        return await edit_delete(
            event, "**𓆜︙ قم بالرد على النص او كتابته مع الامر**"
        )
    video_q = await edit_or_reply(event, "**𓆜︙ يتم البحث في اليوتيوب**")
...
async def kakashi(event):
    ...
    if "www.instagram.com" not in link:
        await edit_or_reply(
            event, "𓆜︙ - يجب كتابة رابط من الانستغرام لتحميله ❕"
        )
    else:
        start = datetime.now()
        catevent = await edit_or_reply(event, "𓆜︙ جار التحميل انتظر قليلا 🔍")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await catevent.edit(" 𓆜︙ قـم بفتح الحظر ع بوت @instasavegrambot")
            return
        await catevent.delete()
