...
async def get_pack_info(event):
    ...
    OUTPUT = (
        f"**𓆝︙ عنوان الملصق:** `{get_stickerset.set.title}\n`"
        f"**𓆝︙ الاسم القصير للملصق:** `{get_stickerset.set.short_name}`\n"
        f"**𓆝︙ المـسؤل:** `{get_stickerset.set.official}`\n"
        f"**𓆝︙ الارشيف:** `{get_stickerset.set.archived}`\n"
        f"**𓆝︙ حزمة الملصق:** `{get_stickerset.set.count}`\n"
        f"**𓆝︙ الايموجي المستخدم**\n{' '.join(pack_emojis)}"
    )
