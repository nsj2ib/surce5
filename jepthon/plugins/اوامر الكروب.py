...
async def kickme(leave):
    "to leave the group."
    await leave.edit("𓆝︙  حسنا سأغادر المجموعه وداعا ")
...
async def _(event):
    ...
    if not result.participant.admin_rights.ban_users:
        return await edit_or_reply(
            event, "𓆝︙ - يبدو انه ليس لديك صلاحيات الحذف في هذه الدردشة "
        )
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            await sleep(0.5)
    await catevent.edit(
        f"𓆝︙  تم بنجاح طرد من {total} الاعضاء ✅ "
    )
...
async def _(event):
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            LOGS.info(str(e))
    await catevent.edit(
        f"𓆝︙  تم بنجاح حظر من {total} الاعضاء ✅ "
    )
...
async def _(event):
    ...
    catevent = await edit_or_reply(
        event, "**𓆝︙ يتـم الـغاء حـظر الجـميع فـي هذه الـدردشـة**"
    )
    succ = 0
    total = 0
    flag = False
    ...
    async for i in event.client.iter_participants(
        ...
        try:
            ...
            try:
                if succ % 10 == 0:
                    await catevent.edit(
                        f"𓆝︙  الغاء حظر جميع الحسابات\nتم الغاء حظر جميع الاعضاء بنجاح ✅"
                    )
            except MessageNotModifiedError:
                pass
    await catevent.edit(f"𓆝︙ الغاء حظر :__{succ}/{total} في الدردشه {chat.title}__")
...
async def rm_deletedacc(show):
    ...
    del_u = 0
    del_status = "𓆝︙  لم يتم العثور على حسابات متروكه او حسابات محذوفة الكروب نظيف"
    if con != "اطردهم":
        event = await edit_or_reply(
            show, "𓆝︙  يتم البحث عن حسابات محذوفة او حسابات متروكة انتظر"
        )
        async for user in show.client.iter_participants(show.chat_id):
            if user.deleted:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"𓆝︙ تـم العـثور : **{del_u}** على حسابات محذوفة ومتروكه في هذه الدردشه من الحسابات في هذه الدردشه,\
    ...
    if not admin and not creator:
        ...
        return
    event = await edit_or_reply(
        show, "𓆝︙ جاري حذف الحسابات المحذوفة"
    )
    del_u = 0
    del_a = 0
    async for user in show.client.iter_participants(show.chat_id):
        if user.deleted:
            try:
                ...
                await edit_delete(event, "𓆝︙  ليس لدي صلاحيات الحظر هنا", 5)
                return
...
async def banall(event):
     ...
     if event.is_private:
         return await edit_or_reply(event, "** 𓆝︙ هذا الامر يستعمل للقنوات والمجموعات فقط !**")
     msg = "حظر"
     is_admin = False
     try:
...
async def ca_sp(event):
  if not event.chat_id in spam_chats:
    return await edit_or_reply(event, "** 𓆝︙ 🤷🏻 لا يوجد طرد او حظر او كتم لأيقافه**")
  else:
    try:
      spam_chats.remove(event.chat_id)
    except:
      pass
    return await edit_or_reply(event, "** 𓆝︙ تم الغاء العملية بنجاح ✓**")
@jepiq.ar_cmd(
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if status:
                    ...
                    await et.edit("𓆝︙  احتاج الى صلاحيات المشرفين للقيام بهذا الامر ")
                    e.append(str(e))
                    break
