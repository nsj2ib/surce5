...
async def fetch_info(replied_user, event):
    ...
    rotbat = "⌁ من مطورين السورس 𓄂𓆃 ⌁" if user_id == 5328713035 else ("⌁ العضـو 𓅫 ⌁")
    rotbat = "⌁ مـالك الحساب 𓀫 ⌁" if user_id == (await event.client.get_me()).id and user_id != 5328713035  else rotbat
    caption = "✛━━━━━━━━━━━━━✛\n"
...
async def _(event):
    ...
    if not replied_user:
        return
    catevent = await edit_or_reply(event, "𓆜︙ جار إحضار معلومات المستخدم اننظر قليلا ⚒️")
...
async def _(event):
    ...
    if input_str:
        ...
        try:
            if p.first_name:
                return await edit_or_reply(
                    event, f"𓆜︙ ايدي المستخدم : `{input_str}` هو `{p.id}`"
                )
        except Exception:
            try:
                if p.title:
                    return await edit_or_reply(
                        event, f"𓆜︙ ايدي الدردشة/القناة `{p.title}` هو `{p.id}`"
                    )
            except Exception as e:
                LOGS.info(str(e))
        await edit_or_reply(event, "𓆜︙ يـجب كـتابة مـعرف الشـخص او الـرد عـليه")
        ...
        if r_msg.media:
            ...
            await edit_or_reply(
                event,
                f"𓆜︙ ايدي الدردشه: `{str(event.chat_id)}` \n𓆜︙ ايدي المستخدم: `{str(r_msg.sender_id)}` \n𓆜︙ ايدي الميديا: `{bot_api_file_id}`",
            )
        else:
            await edit_or_reply(
                event,
        ...
        await edit_or_reply(event, f"𓆜︙ الـدردشـة الـحالية : `{str(event.chat_id)}`")
