...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "الحمد لله دائماً وابداً 🎀 𝑆𝐻 : @SC_60"
)
...
async def _(event):
    ...
    if replied_user.id == 5328713035:
        return await edit_delete(event, "**لا تحاول تنتحل المطورين ادبسز!**")
    if replied_user.id == 5523863949:
        return await edit_delete(event, "**لا تحاول تنتحل المطورين ادبسز!**")
    if replied_user.id == 6099224368:
        return await edit_delete(event, "**لا تحاول تنتحل المطورين ادبسز!**")
...
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        try:
            ...
            return await edit_delete(event, "**𓆜︙ لا يوجد هكذا كروب او قناة تاكد من اليوزر او الايدي ويجب ان يكون/تكون عام/عامة وليس خاص/خاصة**")
        mych = await jepiq(GetFullChannelRequest(
                event.chat_id
            ))
        if msg in jeps:
            return await edit_delete(event, "**𓆜︙ لا يمكنك انتحال قناة او كروب السورس !**")
        ...
        try:
            ...
            return await edit_delete(event, "**𓆜︙ يجب ان تكون لديك صلاحيات لتغيير الاسم والصورة والبايو لانتحال قناة او كروب**")
        ...
        try:
            ...
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        await edit_delete(event, "**𓆜︙ تم الانتحال بنجاح ✓**")
        base64m = 'QGplcHRob24='
        message = base64.b64decode(base64m)
        messageo = message.decode()
        if len(messageo) != 8:
            return await edit_delete(event, "لا تغير الرسالة @L_H_V")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
        ...
        await edit_delete(event, "**𓆜︙ يمكنك انتحال قناة او كروب في قناة او كروب فقط**")
    ...
    if event.is_group or event.is_channel:
        if gvarstatus (f"{event.chat_id}name"):
            try:
                ...
                return await edit_delete(event, "**𓆜︙ يجب ان تكون لديك صلاحيات لتغيير الاسم والصورة والبايو لإعادة القناة او الكروب**")
            except FloodWaitError:
            ...
            await edit_delete(event, "**𓆜︙ تم إعادة الكروب/ القناة بنجاح**")
            delgvar (f"{event.chat_id}name")
            delgvar (f"{event.chat_id}about")
        else:
            await edit_delete(event, "**لم تقم بانتحال قناة او كروب للإعادة**")
    else:
        await edit_delete(event, "**𓆜︙ يمكنك إعادة الدردشة المُنتحِله عبر كتابة الامر في الكروب او القناة المُنتحِله فقط**")
