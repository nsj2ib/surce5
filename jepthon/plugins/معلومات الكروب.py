...
@jepiq.ar_cmd(
    ...
    info={
        ...
        "examples": "{tr} المطور @SC_60",
    },
)
async def _(event):
    "لإظهـار قائمـة المشرفيـن  ✪"
    mentions = "**𓆜︙ مشرفيـن هـذه المجموعـة  ✪**: \n"
    ...
    if input_str:
        ...
        if not event.is_group:
            return await edit_or_reply(event, "**𓆜︙ هـذه ليسـت مجموعـة ✕**")
    try:
...
@jepiq.ar_cmd(
    ...
    info={
        "header": "𓆜︙ لإظهـار قائمـة البوتـات 🝰",
        "description": "𓆜︙ سيظهـر لك قائمـة البوتـات  🝰",
        "usage": [
...
async def _(event):
    "𓆜︙ لإظهـار قائمـة البوتـات 🝰"
    mentions = "**𓆜︙ البـوتات في هذه الـمجموعة 🝰 : ** \n"
    input_str = event.pattern_match.group(1)
    if not input_str:
        chat = await event.get_input_chat()
    else:
        mentions = "**𓆜︙ البوتـات في {} من المجموعات 🝰 : ** \n".format(input_str)
        try:
            chat = await event.client.get_entity(input_str)
        except Exception as e:
...
async def get_users(show):
    ...
    if input_str:
        ...
        if not show.is_group:
            return await edit_or_reply(show, "**𓆜︙ هـذه ليسـت مجموعـة ✕**")
    catevent = await edit_or_reply(show, "**𓆜︙ جـاري سحـب قائمـة معرّفـات الأعضـاء 🝛**")
    try:
        if show.pattern_match.group(1):
            async for user in show.client.iter_participants(chat.id):
                if user.deleted:
                    mentions += f"\n**𓆜︙ الحسـابات المحذوفـة ⌦** `{user.id}`"
                else:
                    mentions += (
...
@jepiq.ar_cmd(
    ...
    info={
        ...
        "examples": "{tr}معلومات @SC_60",
    },
)
async def info(event):
    "𓆜︙ للحصـول على معلومـات المجموعـة 🝢"
    if not event.is_group:
    ...
    try:
        ...
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, f"**𓆜︙ هنـاك خطـأ في معلومـات الدردشـة ✕ : **\n`{str(e)}`"
            )
        await catevent.edit("**𓆜︙ حـدث خـطأ مـا، يرجـى التحقق من الأمـر ⎌**")
...
async def get_chatinfo(event, catevent):
    ...
    try:
        ...
        try:
            ...
            await catevent.edit("**𓆜︙ لـم يتـمّ العثـور على القنـاة/المجموعـة ✕**")
            return None
        except ChannelPrivateError:
            await catevent.edit(
                '**𓆜︙ هـذه مجموعـة أو قنـاة خاصـة أو لقد تمّ حظـري منه ⛞**'
            )
            return None
        except ChannelPublicGroupNaError:
            await catevent.edit("**𓆜︙ القنـاة أو المجموعـة الخارقـة غيـر موجـودة ✕**")
            return None
...
async def fetch_info(chat, event):  # sourcery no-metrics
    ...
    caption = "<b>𓆜︙ معلومـات الدردشـة  🝢 :</b>\n"
    caption += f"⌔︙الآيـدي  : <code>{chat_obj_info.id}</code>\n"
    if chat_title is not None:
        caption += f"𓆜︙ إسـم المجموعـة  :{chat_title}\n"
    if former_title is not None:  # Meant is the very first title
        caption += f"𓆜︙ الإسم السابـق  : {former_title}\n"
    if username is not None:
        caption += f"𓆜︙ نـوع المجموعـة ⌂ : مجموعـة عامّـة  \n"
        caption += f"𓆜︙ الرابـط  : \n {username}\n"
    else:
        caption += f"𓆜︙ نـوع المجموعـة ⌂ : مجموعـة عامّـة  \n"
    if creator_username is not None:
        caption += f"𓆜︙ المالـك  :  {creator_username}\n"
    elif creator_valid:
        caption += (
            '𓆜︙ المالـك  : <a href="tg://user?id={creator_id}">{creator_firstname}</a>\n'
        )
    if created is not None:
        caption += f"𓆜︙ تاريـخ الإنشـاء  : \n <code>{created.date().strftime('%b %d, %Y')} - {created.time()}</code>\n"
    else:
        caption += f"𓆜︙ الإنتـاج  :   <code>{chat_obj_info.date.date().strftime('%b %d, %Y')} - {chat_obj_info.date.time()}</code> {warn_emoji}\n"
    caption += f"⌔︙آ يـدي قاعـدة البيانـات : {dc_id}\n"
    if exp_count is not None:
        chat_level = int((1 + sqrt(1 + 7 * exp_count / 14)) / 2)
        caption += f"𓆜︙ الأعضـاء : <code>{chat_level}</code>\n"
    if messages_viewable is not None:
        caption += f"𓆜︙ الرسائـل التي يمڪن مشاهدتها : <code>{messages_viewable}</code>\n"
    if messages_sent:
        caption += f"𓆜︙ الرسائـل المرسلـة  :<code>{messages_sent}</code>\n"
    elif messages_sent_alt:
        caption += f"𓆜︙ الرسـائل المرسلة: <code>{messages_sent_alt}</code> {warn_emoji}\n"
    if members is not None:
        caption += f"𓆜︙ الأعضـاء : <code>{members}</code>\n"
    if admins is not None:
        caption += f"𓆜︙ المشرفيـن : <code>{admins}</code>\n"
    if bots_list:
        caption += f"𓆜︙ البـوتات : <code>{bots}</code>\n"
    if members_online:
        caption += f"𓆜︙ المتصليـن حـالياً : <code>{members_online}</code>\n"
    if restrcited_users is not None:
        caption += f"𓆜︙ الأعضـاء المقيّديـن : <code>{restrcited_users}</code>\n"
    if banned_users is not None:
        caption += f"𓆜︙ الأعضـاء المحظوريـن : <code>{banned_users}</code>"
    if group_stickers is not None:
        caption += f'{chat_type} ⌔︙الملصقـات : <a href="t.me/addstickers/{chat.full_chat.stickerset.short_name}">{group_stickers}</a>'
    caption += "\n"
    if not broadcast:
        caption += f"𓆜︙ الوضـع البطيئ : {slowmode}"
        if (
            ...
            caption += "\n"
        caption += f"𓆜︙ الـمجموعـة الخارقـة  : {supergroup}\n"
    if hasattr(chat_obj_info, "restricted"):
        caption += f"𓆜︙ المقيّـد : {restricted}"
        if chat_obj_info.restricted:
            caption += f"> : {chat_obj_info.restriction_reason[0].platform}\n"
            caption += f"> 𓆜︙  السـبب  : {chat_obj_info.restriction_reason[0].reason}\n"
            caption += f"> 𓆜︙ النّـص  : {chat_obj_info.restriction_reason[0].text}\n\n"
        else:
            caption += "\n"
    if hasattr(chat_obj_info, "scam") and chat_obj_info.scam:
        caption += "⌔︙السارقيـن : <b>Yes</b>\n"
    if hasattr(chat_obj_info, "verified"):
        caption += f"𓆜︙ الحسابـات الموثقـة   : {verified}\n"
    if description:
