...
async def save_welcome(event):
    ...
    await edit_or_reply("**𓆝︙ هـنالك خـطأ في وضـع الـترحيب هـنا**")
...
async def del_welcome(event):
    ...
    if rm_welcome_setting(event.chat_id) is True:
        await edit_or_reply(event, "**𓆝︙ تم حذف الترحيب بنجاح ✓**")
    else:
        await edit_or_reply(event, "**𓆝︙ ليـس لـدي اي تـرحيبـات بالأصـل ✓**")
...
async def show_welcome(event):
    ...
    cws = get_current_welcome_settings(event.chat_id)
    if not cws:
        return await edit_or_reply(event, "**𓆝︙ لم يتم حفظ اي ترحيب هنا**")
    if cws.f_mesg_id:
        msg_o = await event.client.get_messages(
            entity=BOTLOG_CHATID, ids=int(cws.f_mesg_id)
        )
        await edit_or_reply(
            event, "𓆝︙ أنا الان اقوم بالترحيب بالمستخدمين الجدد مع هذه الرسالة"
        )
